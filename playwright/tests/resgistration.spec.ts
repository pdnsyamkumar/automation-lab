...
test.describe('Registration', async () => {
    ...
    test('Test Case 1: Register User', async ({ homePage, page, signUpPage, topNavigationPage }) => {
        ...
        await test.step(`Step 5: Verify that 'New User Signup!' is visible`, async () => {  
            ...
            await expect(signUpPage.getNewUserSignUpLabel()).toHaveText('New User Signup!');
        })

        await test.step(`Step 6: Enter name and email address`, async () => {
            await signUpPage.fillNameAndEmailAddress(defaultSignUpData.name, defaultSignUpData.email);
        })
        ...
        await test.step(`Step 14: Verify that 'ACCOUNT CREATED!' is visible`, async () => { 
            ...
            await expect(signUpPage.getAccountCreatedLabel()).toHaveText(LabelMessages.ACCOUNT_CREATED);
        })
        ...
        await test.step(`Step 16: Verify that 'Logged in as username' is visible`, async () => {
            await expect(topNavigationPage).toHaveNavOption(`Logged in as ${defaultSignUpData.name}`);
        })
        ...
        await test.step(`Step 18. Verify that 'ACCOUNT DELETED!' is visible and click 'Continue' button`, async () => {
            ...
            await expect(signUpPage.getAccountDeletedLabel()).toHaveText(LabelMessages.ACCOUNT_DELETED);
            await signUpPage.getContinueButton().click();
        })
    })
})