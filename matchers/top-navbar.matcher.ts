...
export const topNavbarMatchers = {
  async toHaveNavOption(
    ...
    try {
      const navOption = await topNavigationPage
        .getNavOptionByLabel(expectedOption)
        .innerText()
  ...
  async toHaveAllNavOptions(
    ...
    const message = () => {
      if (missingOptions.length === 0) {
        return (
          `Expected: All navigation options to be present\n` +
          `Received: All options are present`
        )
      }
      return (
        `Expected: All navigation options: "${expectedOptions.join(', ')}" to be present\n` +
